# fichier pour tout extraure totalement les données depuis le rhd pour tous les types d'xp
...
path = '/mnt/working2/felicie/data2/eTheremin/ALTAI/ALTAI_20240806_SESSION_00/'
session_type = 'Playback' #TrackingOnly ou PbOnly ou MappingChange
...
def compute_psth(spike_times, stimulus_times, bin_size, window):
    ...
    try:
        ...
        stimulus_times = [time / sr for time in stimulus_times]
        spike_times = [time / sr for time in spike_times]
    all_spikes = []
...
for cluster in range(num_plots):
    if cluster < num_plots:
        ...
        try:
            ...
            axes[row, col].plot(psth_bins[:-1], psth)
