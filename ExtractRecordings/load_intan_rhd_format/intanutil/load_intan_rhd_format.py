...
# Modified Flavien Feral June-August 2022
...
def read_data(filename, data_in_float=False, dig_in=True, analog_in=False):
    ...
    if data_present:
        print('File contains {:0.3f} seconds of data. '
              'Amplifiers were sampled at {:0.2f} kS/s.'.format(record_time, header['sample_rate'] / 1000))
    else:
        print('Header file contains no data. '
              'Amplifiers were sampled at {:0.2f} kS/s.'.format(header['sample_rate'] / 1000))
    ...
    if data_present:
        ...
        # if unsigned int values are preferred(0 for False, 1 for True), replace the 'dtype=np.bool'
        # argument with 'dtype=np.uint' as shown the commented line below illustrates this for digital
        # input data; the same can be done for digital out
    ...
    if data_present:
        ...
        # units = microvolts
        # board_adc_data = 312.5e-6
        # amplifier_data = 0.195
        # supply_voltage_data = 74.8e-6
        #
        if data_in_float:
            # ICI: on multiplie par 0.195 les données neuro pour avoir les valeurs en µV
            data['amplifier_data'] = np.multiply(0.195, (data['amplifier_data'].astype(np.int32) - 32768))
        ...
        if header['eval_board_mode'] == 1:
            # units = volts
            data['board_adc_data'] = np.multiply(152.59e-6, (data['board_adc_data'].astype(np.int32) - 32768))
        elif header['eval_board_mode'] == 13:
            # units = volts
            data['board_adc_data'] = np.multiply(312.5e-6, (data['board_adc_data'].astype(np.int32) - 32768))
        else:
        ...
        data['t_supply_voltage'] = data['t_amplifier'][range(0, len(data['t_amplifier']),
        ...
        if header['notch_filter_frequency'] > 0 and header['version']['major'] < 3:
            ...
            for i in range(header['num_amplifier_channels']):
                data['amplifier_data'][i, :] = notch_filter(data['amplifier_data'][i, :],
                                                            header['sample_rate'],
